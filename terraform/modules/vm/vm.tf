resource "azurerm_network_interface" "Azuredevops" {
  name                = "mydevopsvm"
  location            = "eastus"
  resource_group_name = "Azuredevops"

  ip_configuration {
    name                          = "internal"
    subnet_id                     = "azurerm_subnet.internal.id"
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = "/subscriptions/373225e3-4428-4fd7-bd11-c0f187aa1371/resourceGroups/Azuredevops/providers/Microsoft.Compute/virtualMachines/mydevopsvm"
  }
}

resource "azurerm_linux_virtual_machine" "Azuredevops" {
  name                = "myazdevopsvm"
  location            = "eastus"
  resource_group_name = "Azuredevops"
  size                = "Standard_DS2_v3"
  admin_username      = "azureuser"
  address_prefixes    = "10.0.1.0/24"
  network_interface_ids = []
  admin_ssh_key {
    username   = "azureuser"
    public_key = "ssh-rsa  odluser248927@gmail.com"
  }
  os_disk {
    caching           = "ReadWrite"
    storage_account_type = "Standard_LRS"
  }
  source_image_reference {
    publisher = "Canonical"
    offer     = "0001-com-ubuntu-server-focal"
    sku       = "20_04-lts-gen2"
    version   = "latest"
  }
}
